...
7. The **Is\_CW\_Platform\_Installed** task uses the **noop** action to check whether ConnectWise Platform is installed.
8. If ConnectWise Platform is installed, the **Get\_CW\_Platform\_Device\_Counts** task runs the **\[PROD TASK] CW Platform: Get Counts** subworkflow to retrieve device counts from ConnectWise Platform.
...
17. The **check\_rmm\_append** task uses the **noop** action to determine whether any RMM data should be appended by checking if ImmyBot, Datto RMM, ConnectWise Automate, NinjaOne, or ConnectWise Platform is installed.
